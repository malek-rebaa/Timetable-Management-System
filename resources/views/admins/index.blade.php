<x-layout.app>
    ...
    <x-ui.card>
        <x-ui.table :headers="['Nom', 'Email', 'Téléphone', 'Statut', 'Actions']">
            ...
            <tr>
                ...
                <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-400">{{ $admin->phone ?? '—' }}</td>
                <td class="px-5 py-4">
                ...
                <td class="px-5 py-4">
                    <div class="flex gap-2">
                        ...
                        <button 
                            ...
                            x-data 
                            @click="$dispatch('open-modal', 'reset-password-{{ $admin->id }}')">
                        ...
                        <button 
                            ...
                            title="Supprimer"
                            x-data 
                            @click="$dispatch('open-modal', 'delete-admin-{{ $admin->id }}')">
            ...
            {{-- Edit Modal --}}
            <x-ui.modal name="edit-admin-{{ $admin->id }}" title="Modifier l'Admin" maxWidth="lg">
                <form action="{{ route('admins.update', $admin) }}" method="POST" class="flex flex-col gap-4">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Prénom</label>
                        <x-form.input type="text" name="first_name" value="{{ $admin->first_name }}" required />
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Nom</label>
                        <x-form.input type="text" name="last_name" value="{{ $admin->last_name }}" required />
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Téléphone</label>
                        <x-form.input type="text" name="phone" value="{{ $admin->phone }}" placeholder="+216 XX XXX XXX" />
                    </div>
                    <div class="flex justify-end gap-3 mt-2">
                        <x-form.button type="button" variant="secondary" @click="$dispatch('close-modal', 'edit-admin-{{ $admin->id }}')">
                            Annuler
                        </x-form.button>
            ...
            <x-ui.modal name="reset-password-{{ $admin->id }}" title="Réinitialiser le mot de passe" maxWidth="md">
                <form action="{{ route('admins.reset-password', $admin) }}" method="POST" class="flex flex-col gap-4">
                    @csrf
                    @method('PUT')
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        Êtes-vous sûr de vouloir réinitialiser le mot de passe de <strong>{{ $admin->first_name }} {{ $admin->last_name }}</strong> ?
                    </p>
                    <p class="text-xs text-warning-600 dark:text-warning-400">
                        Un nouveau mot de passe sécurisé sera généré et affiché une seule fois.
                    </p>
                    <div class="flex justify-end gap-3 mt-2">
                        <x-form.button type="button" variant="secondary" @click="$dispatch('close-modal', 'reset-password-{{ $admin->id }}')">
                            Annuler
                        </x-form.button>
                        <x-form.button type="submit" variant="primary">
                            Réinitialiser
            ...
            <x-ui.modal name="delete-admin-{{ $admin->id }}" title="Confirmer la suppression" maxWidth="md">
                <form action="{{ route('admins.destroy', $admin) }}" method="POST" class="flex flex-col gap-4">
                    @csrf
                    @method('DELETE')
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        Êtes-vous sûr de vouloir supprimer <strong>{{ $admin->first_name }} {{ $admin->last_name }}</strong> ?
                    </p>
                    <p class="text-xs text-error-600 dark:text-error-400">Cette action est irréversible.</p>
                    <div class="flex justify-end gap-3 mt-2">
                        <x-form.button type="button" variant="secondary" @click="$dispatch('close-modal', 'delete-admin-{{ $admin->id }}')">
                            Annuler
                        </x-form.button>
    ...
    {{-- Create Admin Modal --}}
    <x-ui.modal name="create-admin" title="Ajouter un Administrateur" maxWidth="lg">
        <form action="{{ route('admins.store') }}" method="POST" class="flex flex-col gap-4">
            @csrf
            <div>
                <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Prénom</label>
            ...
            <div class="flex justify-end gap-3 mt-2">
                <x-form.button type="button" variant="secondary" @click="$dispatch('close-modal', 'create-admin')">
                    Annuler
                </x-form.button>
                <x-form.button type="submit" variant="primary">
                    Créer l'Admin
